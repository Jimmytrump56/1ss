...
exports.renderASTNode = exports.RenderType = void 0;
...
async function renderASTNode(node, context) {
    ...
    switch (type) {
        ...
        case 'channel': {
            ...
            return (react_1.default.createElement(discord_components_react_1.DiscordMention, { type: channel ? (channel.isTextBased() ? (channel.isThread() ? 'thread' : 'channel') : 'voice') : 'channel' }, channel ? (channel.isDMBased() ? 'DM Channel' : channel.name) : `Unknown Channel (${id})`));
        }
        case 'role': {
            const id = node.id;
            const role = await context.callbacks.resolveRole(id);
            return (react_1.default.createElement(discord_components_react_1.DiscordMention, { type: "role", color: context.type === RenderType.REPLY ? undefined : role === null || role === void 0 ? void 0 : role.hexColor }, role ? role.name : `Unknown Role (${id})`));
        }
        case 'user': {
            const id = node.id;
            const user = await context.callbacks.resolveUser(id);
            return react_1.default.createElement(discord_components_react_1.DiscordMention, { type: "user" }, user ? user.username : `Unknown User (${id})`);
        }
        case 'here':
        case 'everyone':
            return (react_1.default.createElement(discord_components_react_1.DiscordMention, { type: 'role', highlight: true }, type === 'here' ? '@here' : '@everyone'));
        case 'codeBlock':
            if (context.type !== RenderType.REPLY) {
